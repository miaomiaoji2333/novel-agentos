...
import { Book, Layout, Globe, FileText, Settings, Plus, Trash2, HelpCircle, Link, Check, Sparkles, ChevronDown, ChevronRight, Folder, Home, Moon, Sun, Puzzle, LogOut, Shield, UserCircle } from 'lucide-react';
    ...
    <div className="w-64 h-full bg-paper-50 dark:bg-zinc-900 border-r border-paper-200 dark:border-zinc-800 flex flex-col flex-shrink-0 relative transition-colors duration-300">
      ...
      <div className="p-3 flex-1 overflow-y-auto custom-scrollbar">
        <div className="mb-8">
          ...
          {project && (
            <>
              <button onClick={() => setViewMode(ViewMode.WRITER)} className={navItemClass(viewMode === ViewMode.WRITER)}>
                <FileText className="w-4 h-4 mr-3" /> 写作手稿
              </button>
              <button onClick={() => setViewMode(ViewMode.PLANBOARD)} className={navItemClass(viewMode === ViewMode.PLANBOARD)}>
                <Layout className="w-4 h-4 mr-3" /> 情节大纲
              </button>
              <button onClick={() => setViewMode(ViewMode.WORLD)} className={navItemClass(viewMode === ViewMode.WORLD)}>
                <Globe className="w-4 h-4 mr-3" /> 世界观档案
              </button>
              <button onClick={() => setViewMode(ViewMode.PLUGINS)} className={navItemClass(viewMode === ViewMode.PLUGINS)}>
