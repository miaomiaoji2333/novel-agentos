...
import { pingPlugin } from '../services/pluginService';
import { 
...
export const PluginManager: React.FC = () => {
  ...
  const handlePing = async (id: string) => {
    ...
    const result = await pingPlugin(plugin.endpoint);
    
    if (result) {
      setProject({
        ...project,
        plugins: plugins.map(p => p.id === id ? { ...p, status: 'online', latency: result.latency, lastPing: Date.now() } : p)
      });
      addLog(`${plugin.name} is active (${result.latency}ms)`, 'success');
    } else {
      setProject({
        ...project,
        plugins: plugins.map(p => p.id === id ? { ...p, status: 'offline' } : p)
      });
      addLog(`${plugin.name} connection failed`, 'error');
    }
  };
  ...
  const handleConnectPlugin = async () => {
    ...
    const result = await pingPlugin(newEndpoint);
    if (result && result.manifest.id) {
      const { manifest, latency } = result;
      ...
      if (exists) {
        setProject({
          ...project,
          plugins: plugins.map(p => p.id === newPlugin.id ? { ...newPlugin, config: p.config || {} } : p)
        });
        addLog(`Plugin [${newPlugin.name}] re-synced successfully.`, 'success');
      } else {
        setProject({
          ...project,
          plugins: [...plugins, newPlugin]
        });
        addLog(`Installed new plugin: ${newPlugin.name}`, 'success');
      }
      setNewEndpoint('');
    } else {
      addLog(`Handshake failed. Ensure endpoint is reachable and CORS is enabled.`, 'error');
    }
