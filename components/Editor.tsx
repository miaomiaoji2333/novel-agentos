...
import { 
  ...
  AlignLeft, Save, Edit3, FileText, Hash, Puzzle, Play, ShieldAlert, ChevronRight, AlertCircle, Settings
} from 'lucide-react';
...
export const Editor: React.FC = () => {
  ...
  useEffect(() => {
    if (activeDoc) {
      ...
      if (!activeDoc.timeNode && activeDoc.title && !isGeneratingTime) {
  ...
  const handleAIRequest = async () => {
    if (!activeDoc || activeDoc.status === '完成') return;
            ...
            <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[90%] max-w-lg z-40">
                <div className="bg-white/90 dark:bg-zinc-900/90 backdrop-blur-2xl shadow-2xl border border-paper-200 dark:border-white/5 rounded-2xl p-1.5 flex flex-col gap-2 ring-1 ring-black/5 transition-all">
                    <div className="flex items-center justify-between px-2 pt-1">
                         ...
                         <div className="text-[9px] font-black text-ink-300 dark:text-zinc-700 uppercase px-2 tracking-widest hidden md:block">AI Ghostwriter</div>
                    </div>
